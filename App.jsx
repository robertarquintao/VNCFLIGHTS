...
import {
  collection, addDoc, onSnapshot, updateDoc, doc, orderBy, query
...
const TRUST_BADGES = ["Melhor Preco Garantido","Resposta em ate 2h","100% Seguro","Destinos no Mundo Todo"];
const TESTIMONIALS = [
  { name:"Ana Paula", city:"Rio de Janeiro", text:"Consegui um voo para Lisboa 40% mais barato! Atendimento incrivel." },
  { name:"Carlos M.", city:"Sao Paulo", text:"Viagem dos sonhos para Orlando com tudo incluido. Super recomendo!" },
  { name:"Fernanda L.", city:"Belo Horizonte", text:"Rapido, facil e muito barato. Nunca mais vou buscar passagem sozinha." },
];

function SearchButtons({ req }) {
  function googleFlightsUrl() {
    const orig = encodeURIComponent(req.origin.replace(/\(.*\)/,"").trim());
    const dest = encodeURIComponent(req.destination.replace(/\(.*\)/,"").trim());
    return "https://www.google.com/travel/flights?q=Flights+" + orig + "+" + dest + "&curr=BRL";
  }
  const airlines = [
    { name:"LATAM",     color:"#E31837", url:"https://www.latamairlines.com/br/pt/oferta-voos" },
    { name:"Azul",      color:"#0066CC", url:"https://www.voeazul.com.br" },
    { name:"Gol",       color:"#FF6600", url:"https://www.voegol.com.br" },
    { name:"American",  color:"#0078D2", url:"https://www.aa.com" },
    { name:"Decolar",   color:"#00A0E0", url:"https://www.decolar.com" },
    { name:"Submarino", color:"#6B46C1", url:"https://viagens.submarino.com.br" },
  ];
  return (
    <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"16px 18px",marginBottom:20 }}>
      <p style={{ fontSize:11,fontWeight:700,color:"#475569",letterSpacing:"0.1em",margin:"0 0 12px" }}>BUSCA RAPIDA DE PRECOS</p>
      <a href={googleFlightsUrl()} target="_blank" rel="noreferrer"
        style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,width:"100%",padding:"10px",borderRadius:9,background:"linear-gradient(135deg,#4285f4,#34a853)",color:"#fff",fontWeight:700,fontSize:14,textDecoration:"none",marginBottom:10,boxSizing:"border-box" }}>
        Buscar no Google Flights
      </a>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8 }}>
        {airlines.map((a)=>(
          <a key={a.name} href={a.url} target="_blank" rel="noreferrer"
            style={{ display:"flex",alignItems:"center",justifyContent:"center",padding:"8px 4px",borderRadius:8,border:"1px solid rgba(255,255,255,0.08)",background:"rgba(255,255,255,0.03)",color:"#f1f5f9",fontWeight:600,fontSize:12,textDecoration:"none",textAlign:"center" }}>
            <span style={{ width:8,height:8,borderRadius:"50%",background:a.color,display:"inline-block",marginRight:6,flexShrink:0 }}/>
            {a.name}
          </a>
        ))}
      </div>
...
function ClientPage({ onSubmit }) {
  ...
  async function submit() {
    ...
    setSaving(true);
    try {
      await onSubmit({ ...form,status:"pending",createdAt:new Date().toLocaleString("pt-BR") });
      setDone(true);
  ...
  if (done) return (
    <div style={{ minHeight:"100vh",background:"linear-gradient(135deg,#0a0f1e,#0d1f3c)",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div style={{ background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:24,padding:"56px 48px",textAlign:"center",maxWidth:480 }}>
        <div style={{ width:80,height:80,background:"linear-gradient(135deg,#10b981,#059669)",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,fontWeight:800,color:"#fff",margin:"0 auto 24px" }}>OK</div>
    ...
    <div style={{ minHeight:"100vh",background:"linear-gradient(135deg,#0a0f1e,#0d1f3c)",fontFamily:"'Segoe UI',sans-serif" }}>
      <div style={{ position:"relative",overflow:"hidden",padding:"60px 20px 50px",textAlign:"center" }}>
        ...
        <div style={{ display:"flex",flexWrap:"wrap",gap:12,justifyContent:"center" }}>
          {TRUST_BADGES.map((b)=>(
            <div key={b} style={{ display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:100,padding:"6px 14px" }}>
              <span style={{ color:"#cbd5e1",fontSize:13,fontWeight:500 }}>{b}</span>
            </div>
          ))}
        </div>
      </div>
      <div style={{ maxWidth:720,margin:"0 auto",padding:"0 20px 20px" }}>
        <div style={{ background:"rgba(255,255,255,0.04)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:24,padding:"clamp(20px,4vw,40px)" }}>
          <div style={{ background:"linear-gradient(135deg,rgba(14,165,233,0.15),rgba(99,102,241,0.15))",border:"1px solid rgba(14,165,233,0.2)",borderRadius:12,padding:"12px 18px",marginBottom:28 }}>
          ...
          <button style={{ ...s.btnCTA,opacity:saving?0.7:1 }} onClick={submit} disabled={saving}>
            {saving ? "Enviando..." : "Quero Meu Orcamento Gratis no WhatsApp"}
          </button>
...
function VoucherForm({ request, quote, onNext, onCancel }) {
  const [v, setV] = useState({
    passengerName: request.name, phone: request.phone,
    origin: request.origin, destination: request.destination,
    departDate: request.departDate, returnDate: request.returnDate||"",
    adults: request.adults, children: request.children, cabinClass: request.cabinClass,
    airline: quote.airline||"", departure: quote.departure||"", arrival: quote.arrival||"",
    stops: quote.stops||"0", baggage: quote.baggage||"",
    reservationNumber: "", salePrice: quote.price||"", voucherNotes: "",
  });
...
function FinancialForm({ voucherData, onSave, onBack }) {
  const [f, setF] = useState({ costPrice:"",salePrice:voucherData.salePrice||"",profit:"",issuedBy:"",paymentMethod:"pix",status:"pago" });
  const [saving, setSaving] = useState(false);
  function handle(e) {
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      await onSave({ ...voucherData,...f,closedAt:new Date().toLocaleString("pt-BR"),saleDate:new Date().toISOString().split("T")[0] });
  ...
  return (
    <div style={{ maxWidth:700 }}>
      ...
      <Row>
        <Field label="Forma de Pagamento">
          <select style={s.input} name="paymentMethod" value={f.paymentMethod} onChange={handle}>
            <option value="pix">PIX</option><option value="credito">Cartao de Credito</option>
            <option value="debito">Cartao de Debito</option><option value="transferencia">Transferencia</option>
            <option value="dinheiro">Dinheiro</option>
          </select>
        </Field>
        <Field label="Status">
          <select style={s.input} name="status" value={f.status} onChange={handle}>
            <option value="pago">Pago</option><option value="pendente">Pendente</option><option value="parcelado">Parcelado</option>
          </select>
        </Field>
      </Row>
      {f.profit && (
      ...
      <div style={{ display:"flex",gap:12,marginTop:24 }}>
        ...
        <button style={{ ...s.btnGreen,flex:1,padding:"13px",fontSize:15,opacity:saving?0.7:1 }} onClick={save} disabled={saving}>
          {saving?"Salvando...":"Salvar no Relatorio Financeiro"}
        </button>
      </div>
    </div>
  );
...
function StepIndicator({ current }) {
  return (
    <div style={{ display:"flex",alignItems:"center",marginBottom:28 }}>
      {[["1","Voucher"],["2","Financeiro"]].map(([num,label],i)=>(
        <div key={num} style={{ display:"flex",alignItems:"center" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <div style={{ width:32,height:32,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:14,
              background:parseInt(num)<current?"#10b981":parseInt(num)===current?"linear-gradient(135deg,#0ea5e9,#6366f1)":"rgba(255,255,255,0.06)",
              color:parseInt(num)<=current?"#fff":"#475569" }}>
              {parseInt(num)<current?"OK":num}
            </div>
            <span style={{ fontSize:13,fontWeight:600,color:parseInt(num)<=current?"#f1f5f9":"#475569" }}>{label}</span>
          </div>
          {i===0 && <div style={{ width:40,height:1,background:current>1?"#0ea5e9":"#1e3a5f",margin:"0 12px" }}/>}
        </div>
      ))}
    </div>
  );
    ...
    <div>
      ...
      {sales.length===0 ? (
        <div style={{ textAlign:"center",padding:"60px 0",color:"#334155" }}>
          <p style={{ marginTop:12,fontSize:15 }}>Nenhuma venda registrada ainda.</p>
        </div>
      ) : (
        <div style={{ overflowX:"auto" }}>
          <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
            ...
            <tbody>
              {sales.map((sale,i)=>(
                <tr key={sale.id||i} style={{ borderBottom:"1px solid rgba(255,255,255,0.04)",background:i%2===0?"rgba(255,255,255,0.01)":"transparent" }}>
...
function AgentPage({ requests, onMarkSent, onMarkConfirmed, sales, onAddSale }) {
  ...
  async function sendWhatsApp() {
    ...
    await onMarkSent(selected.id, q);
    setSelected(null);
  ...
  async function handleFinancialSave(sale) {
    await onAddSale(sale, selected?.id);
    setClosingStep(null); setVoucherData(null); setSelected(null); setAgentTab("report");
  }
          ...
          <div style={{ flex:1,overflowY:"auto",padding:28 }}>
            {!selected ? (
              ...
              <div style={{ maxWidth:680 }}>
                <div style={{ background:"rgba(14,165,233,0.06)",border:"1px solid rgba(14,165,233,0.15)",borderRadius:16,padding:20,marginBottom:16 }}>
                  ...
                  <p style={{ margin:"0 0 4px",color:"#64748b",fontSize:13 }}>{selected.adults} adulto(s){selected.children>0?", "+selected.children+" crianca(s)":""} - {CLASS_LABEL[selected.cabinClass]}</p>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const qSales = query(collection(db,"sales"), orderBy("closedAt","desc"));
    const unsubSales = onSnapshot(qSales, (snap) => {
      setSales(snap.docs.map(d=>({ id:d.id,...d.data() })));
    });
    return () => { unsubReq(); unsubSales(); };
  }, []);

  async function addRequest(req) {
    await addDoc(collection(db,"requests"), req);
  }
  async function markSent(id, quote) {
    await updateDoc(doc(db,"requests",id), { status:"sent", savedQuote:quote });
  }
  async function markConfirmed(id) {
    await updateDoc(doc(db,"requests",id), { status:"confirmed" });
  }
  async function addSale(sale, reqId) {
    await addDoc(collection(db,"sales"), sale);
    if (reqId) await updateDoc(doc(db,"requests",reqId), { status:"closed" });
  }
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh",background:"#0a0f1e",display:"flex",alignItems:"center",justifyContent:"center" }}>
      <div style={{ textAlign:"center" }}>
        <div style={{ width:48,height:48,border:"3px solid #1e3a5f",borderTop:"3px solid #0ea5e9",borderRadius:"50%",animation:"spin 1s linear infinite",margin:"0 auto 16px" }}/>
        <p style={{ color:"#475569",fontFamily:"sans-serif" }}>Carregando VNC Flights...</p>
