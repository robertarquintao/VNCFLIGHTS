...
function SearchButtons({ req }) {
  function buildGoogleFlightsUrl() {
    const orig = req.origin.replace(/\(.*\)/,"").trim().replace(/ /g,"+");
    const dest = req.destination.replace(/\(.*\)/,"").trim().replace(/ /g,"+");
    const dep = req.departDate.replace(/-/g,"");
    const ret = req.returnDate ? req.returnDate.replace(/-/g,"") : "";
    const trip = req.returnDate ? "r" : "o";
    return "https://www.google.com/travel/flights?q=Flights+" + orig + "+" + dest + "+" + dep + (ret ? "+" + ret : "") + "&curr=BRL";
  }
  function buildDecolarUrl() {
    const orig = req.origin.replace(/\s*\(.*\)\s*/g,"").trim();
    const dest = req.destination.replace(/\s*\(.*\)\s*/g,"").trim();
    return "https://www.decolar.com/passagens-aereas/" + encodeURIComponent(orig) + "/" + encodeURIComponent(dest) + "/" + req.departDate + (req.returnDate ? "/" + req.returnDate : "") + "/1/0/0";
  }
  const airlines = [
    { name:"LATAM", color:"#E31837", url:"https://www.latamairlines.com/br/pt/oferta-voos?origin="+req.origin.match(/\(([^)]+)\)/)?.[1]+"&destination="+req.destination.match(/\(([^)]+)\)/)?.[1]+"&outbound="+req.departDate+"&adt="+req.adults+"&chd="+req.children+"&inf=0&trip="+(req.returnDate?"RT":"OW")+"&cabin=Y&redemption=false&sort=RECOMMENDED" },
    { name:"Azul", color:"#0066CC", url:"https://www.voeazul.com.br/br/pt/home/selecionar-voo?c="+req.origin.match(/\(([^)]+)\)/)?.[1]+":"+req.destination.match(/\(([^)]+)\)/)?.[1]+":"+req.departDate+":"+req.adults+":0:0:EC" },
    { name:"Gol", color:"#FF6600", url:"https://www.voegol.com.br/passagens-aereas" },
    { name:"American", color:"#0078D2", url:"https://www.aa.com/booking/find-flights" },
    { name:"Decolar", color:"#00A0E0", url:buildDecolarUrl() },
    { name:"Submarino", color:"#6B46C1", url:"https://viagens.submarino.com.br/passagens-aereas" },
  ];
  return (
    <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"16px 18px",marginBottom:20 }}>
      <p style={{ fontSize:11,fontWeight:700,color:"#475569",letterSpacing:"0.1em",margin:"0 0 14px" }}>BUSCA RAPIDA DE PRECOS</p>
      <a href={buildGoogleFlightsUrl()} target="_blank" rel="noreferrer"
        style={{ display:"flex",alignItems:"center",justifyContent:"center",gap:8,width:"100%",padding:"10px",borderRadius:9,background:"linear-gradient(135deg,#4285f4,#34a853)",color:"#fff",fontWeight:700,fontSize:14,textDecoration:"none",marginBottom:10 }}>
